...
static bool himax_mcu_dd_clk_set(bool enable)
{
	...
	data[0] = (enable)?1:0;
	return (g_core_fp.fp_register_write(pic_op->adr_osc_en, sizeof(pic_op->adr_osc_en), data, 0) == NO_ERR);
}
...
static void himax_mcu_dd_reg_en(bool enable)
{
	...
	if (data[0] != 0x44) { /*need DD Touch PW*/
		...
		g_core_fp.fp_register_write(pic_op->adr_osc_pw, DATA_LEN_4, data, 0);
...
static bool himax_mcu_dd_reg_read(uint8_t addr, uint8_t pa_num, int len, uint8_t *data, uint8_t bank)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0))
			goto READ_FAIL;
		data[i] = tmp_data[0];
...
static int himax_mcu_ulpm_in(void)
{
	...
	do {
		...
		if (himax_bus_write(pfw_op->addr_ulpm_34[0], pfw_op->data_ulpm_11, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			I("%s: spi write fail!\n", __func__);
			continue;
		}
		if (himax_bus_read(pfw_op->addr_ulpm_34[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
		}
	...
	do {
		...
		if (himax_bus_write(pfw_op->addr_ulpm_33[0], pfw_op->data_ulpm_33, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			I("%s: spi write fail!\n", __func__);
			continue;
		}
		if (himax_bus_read(pfw_op->addr_ulpm_33[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
		}
	...
	do {
		...
		if (himax_bus_write(pfw_op->addr_ulpm_34[0], pfw_op->data_ulpm_22, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			I("%s: spi write fail!\n", __func__);
			continue;
		}
		if (himax_bus_read(pfw_op->addr_ulpm_34[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
	...
	do {
		...
		if (himax_bus_write(pfw_op->addr_ulpm_33[0], pfw_op->data_ulpm_aa, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			I("%s: spi write fail!\n", __func__);
			continue;
		}
		if (himax_bus_read(pfw_op->addr_ulpm_33[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
...
static int himax_mcu_get_max_dc(void)
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_max_dc, DATA_LEN_4, tmp_data, 0);
	I("%s: tmp_data[0]=%x,tmp_data[1]=%x\n", __func__, tmp_data[0], tmp_data[1]);
...
static int hx_mcu_diff_overlay_bin(void)
{
	int rslt = 0;
	int diff_val = 0;
...
static bool himax_mcu_flash_lastdata_check(uint32_t size)
{
	...
	start_addr = size - flash_page_len; /* In order to match other size of fw*/
	I("%s: Now size is %d, the start_addr is 0x%04X\n", __func__, size, start_addr);
...
static bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	/* 4. FW stop outputing */
	tmp_data[3] = temp_info_data[3]; tmp_data[2] = temp_info_data[2];
...
static uint8_t himax_mcu_tp_info_check(void)
{
	...
	uint16_t x_res = pdriver_op->data_df_x_res[1] << 8 | pdriver_op->data_df_x_res[0];
	uint16_t y_res = pdriver_op->data_df_y_res[1] << 8 | pdriver_op->data_df_y_res[0];
	uint8_t err_cnt = 0;
	...
	if (ic_data->HX_Y_RES < (y_res / 2) || ic_data->HX_Y_RES > (y_res * 3 / 2)) {
		ic_data->HX_Y_RES = y_res;
		err_cnt |= 0x08;
	}
	if (ic_data->HX_X_RES < (x_res / 2) || ic_data->HX_X_RES > (x_res * 3 / 2)) {
...
static int hx_read_guest_info(void)
{
	...
	flash_tmp_buffer = kcalloc(HX_GUEST_INFO_SIZE * flash_page_len, sizeof(uint8_t), GFP_KERNEL);
...
void himax_mcu_write_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, uint32_t write_len)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			...
			tmp_data = (uint8_t *)&fw_entry->data[start_index+i * max_bus_size];
			g_core_fp.fp_register_write(tmp_addr,  max_bus_size, tmp_data, 0);
			total_size_temp = total_size_temp - max_bus_size;
		} else {
...
int hx_parse_bin_cfg_data(const struct firmware *fw_entry, struct zf_info *zf_info_arr, uint8_t *FW_buf, uint8_t *sram_min, uint8_t *ovl_sidx)
{
	...
	for (i = 1; i < part_num; i++) {
		...
		/*overlay section*/
		if (zf_info_arr[i].cfg_addr == ovl_isram_addr) {
			if (*ovl_sidx == 0)
				*ovl_sidx = i;
...
static void himax_mcu_fp_init(void)
{
	...
	g_core_fp._diff_overlay_bin = hx_mcu_diff_overlay_bin;
#endif
#endif
#ifdef CORE_FLASH
...
#endif
#ifndef __MTK_SPI__
	g_internal_buffer = kzalloc(sizeof(uint8_t)*(1024 * 48 + ADDR_LEN_4), GFP_KERNEL);
#else
...
EXPORT_SYMBOL(himax_in_parse_assign_cmd);
...
void himax_mcu_in_cmd_init(void)
{
	...
	himax_in_parse_assign_cmd(fw_addr_max_dc, pfw_op->addr_max_dc, sizeof(pfw_op->addr_max_dc));
	...
	himax_in_parse_assign_cmd(driver_data_df_rx, pdriver_op->data_df_rx, sizeof(pdriver_op->data_df_rx));
	himax_in_parse_assign_cmd(driver_data_df_tx, pdriver_op->data_df_tx, sizeof(pdriver_op->data_df_tx));
	himax_in_parse_assign_cmd(driver_data_df_pt, pdriver_op->data_df_pt, sizeof(pdriver_op->data_df_pt));
	himax_in_parse_assign_cmd(driver_data_df_x_res, pdriver_op->data_df_x_res, sizeof(pdriver_op->data_df_x_res));
	himax_in_parse_assign_cmd(driver_data_df_y_res, pdriver_op->data_df_y_res, sizeof(pdriver_op->data_df_y_res));
