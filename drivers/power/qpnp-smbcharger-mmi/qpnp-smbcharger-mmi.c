...
int smblib_get_aicl_cont_threshold_mmi(struct smb_mmi_chg_param *param, u8 val_raw)
{
...
int smblib_set_aicl_cont_threshold_mmi(struct smb_mmi_chg_param *param,
...
static struct smb_mmi_params smb5_pm8150b_params = {
	...
	.aicl_cont_threshold		= {
		...
		.max_u  = 11800,
		.step_u = 100,
		.get_proc = smblib_get_aicl_cont_threshold_mmi,
		.set_proc = smblib_set_aicl_cont_threshold_mmi,
	},
};
...
int smblib_set_usb_suspend_mmi(struct smb_mmi_charger *chg, bool suspend)
{
	int rc = 0;
...
int smblib_get_usb_suspend_mmi(struct smb_mmi_charger *chg, int *suspend)
{
	int rc = 0;
	u8 temp = 0;
					...
					const char *buf, size_t count)
{
	...
	r = smblib_set_usb_suspend_mmi(mmi_chip, (bool)mode);
					...
					char *buf)
{
	...
	ret = smblib_get_usb_suspend_mmi(mmi_chip, &state);
	if (ret) {
...
int smblib_set_charge_param_mmi(struct smb_mmi_charger *chg,
...
int smblib_get_charge_param_mmi(struct smb_mmi_charger *chg,
			    struct smb_mmi_chg_param *param, int *val_u)
{
	int rc = 0;
	u8 val_raw;
					...
					char *buf)
{
	...
	ret = smblib_get_charge_param_mmi(mmi_chip, &mmi_chip->param.fcc, &state);
	if (ret < 0) {
					...
					char *buf)
{
	...
	r = smblib_get_charge_param_mmi(mmi_chip, &mmi_chip->param.usb_icl, &state);
	if (r < 0) {
					...
					const char *buf, size_t count)
{
	...
	r = smblib_set_charge_param_mmi(mmi_chip, &mmi_chip->param.dc_icl, dc_curr);
	if (r < 0) {
					...
					char *buf)
{
	...
	r = smblib_get_charge_param_mmi(mmi_chip, &mmi_chip->param.dc_icl, &state);
	if (r < 0) {
...
static int mmi_increase_vbus_power(struct smb_mmi_charger *chg, int cur_mv)
{
	...
	if (pulse_cnt == 0) {
		...
		rc = smblib_set_charge_param_mmi(chg, &chg->param.aicl_cont_threshold,
...
static void mmi_heartbeat_work(struct work_struct *work)
{
	...
	rc = smblib_get_usb_suspend_mmi(chip, &usb_suspend);
	if (rc < 0) {
		usb_suspend = 0;
	...
	if (!usb_suspend &&
		...
		if (((chg_stat.usb_mv < REV_BST_THRESH) &&
			...
			if (chip->factory_mode)
				smblib_set_usb_suspend_mmi(chip, true);
			else
				vote(chip->usb_icl_votable, BOOST_BACK_VOTER,
				     true, 0);
			msleep(50);
			if (chip->factory_mode)
				smblib_set_usb_suspend_mmi(chip, false);
			else
				vote(chip->usb_icl_votable, BOOST_BACK_VOTER,
				     false, 0);
		} else {
			 ...
			 unsigned long event, void *unused)
{
	...
	if (chg->factory_mode) {
		...
		case SYS_POWER_OFF:
			...
			/* Suspend USB and DC */
			smblib_set_usb_suspend_mmi(chg, true);
			rc = power_supply_get_property(chg->usb_psy,
...
static int smb_mmi_probe(struct platform_device *pdev)
{
	...
	if (chip->dc_cl_ma >= 0) {
		rc = smblib_set_charge_param_mmi(chip, &chip->param.dc_icl,
					chip->dc_cl_ma * 1000);
		if (rc)
	...
	if (chip->factory_mode) {
		...
		rc = smblib_set_charge_param_mmi(chip, &chip->param.usb_icl,
					     3000000);
		if (rc < 0)
			mmi_err(chip,
